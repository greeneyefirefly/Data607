...
We process information quickly when it is presented using charts or graphs. Thus, it has become imperative to show complex data and report through graphs and charts. In this assignment, the task is to use one or more `tidyverse` packages, and any data set and create a programming sample “vignette” that demonstrates how to use one or more of the capabilities of the selected `tidyverse` package with your selected data set. Therefore, my aim it to demonstrate how `tidyverse` can aid in basic data analysis but still capture valuable information a data set possess. For instance, if I use `ggplot2`, I want to demonstrate how a basic plot can provide a clear, analytical representation of the data in an easily understandable way. 
...
The data set used in this `tidyverse` exploration is the Audio Features obtained from Spotify API about tracks by the Korean boy band called *SHINee*. There are 9 feature which are utilized in this assignment. These include:
...
```{r eval=FALSE}
# Even though the data was inital split, we can easily join them again using 
...
With these plots, the difference in distribution can be observed for both languages. It is apparent that a Japanese track will have a consistently higher energy, lower acousticness, and inconsistant in valence. Whereas, Korean tracks are of wider range of danceability, which is also apparent in the tempo density plot. Lastly, in spite of the language, both kind of tracks has similar speechiness levels.
...
Firstly, `tidyr::gather` is used to gather columns into rows, transformation the wide data of average Korean and Japanese features into a long set. This allows graphing of all the features on one plot for an easier comparison. 
...
From the line graph on SHINee's Korean musical attributes, it is apparent that Korean tracks have reduced tremendously in energy and valence from 2013 to 2016 before picking up again. Moreover, there were very small fluctuation in danceability and speechiness over their career. Conversely, from the line graph on SHINee's Japanese musical attributes, it is apparent that Japanese tracks have steadly reduced in valence and acousticness since 2013-2014. Liveness began increasing again in 2015 after it's reduction over the previous two years. Moreover, there was only very small fluctuation in speechiness over their career, similar to their Korean tracks. 
...
As a fan, I am able to see, or literally hear, how these distinct cluster were formed. The dendrogram depicts five clusters in which I can see the following pattern:
...
* Cluster 3: SHINee's Brand, the main branch shows ablums that has a mix of different mood features from high danceability tracks to low energy. Additionally, the mini-branches also suggest insightful patterns which can be heard which I will not discuss. 
...
colors_border <- c(adjustcolor("forestgreen",alpha.f=0.5), adjustcolor("darkgoldenrod2",alpha.f=0.5), adjustcolor("firebrick4",alpha.f=0.5), adjustcolor("navy",alpha.f=0.5), adjustcolor("hotpink2",alpha.f=0.5))

colors_in <- c(adjustcolor("forestgreen",alpha.f=0.5), adjustcolor("darkgoldenrod2",alpha.f=0.5), adjustcolor("firebrick4",alpha.f=0.5), adjustcolor("navy",alpha.f=0.5), adjustcolor("hotpink2",alpha.f=0.5))

# fmsb::radarchart(radarsummary[,-1] , axistype=0 , maxmin=F, pcol=colors_border , pfcol=colors_in , plwd=4 , plty=1, cglcol="grey", cglty=1, axislabcol="black", cglwd=0.8, vlcex=0.8)
# legend("bottomleft", legend = c("#1: Live Concerts", "#2: Peak Time", "#3: SHINee Brand", "#4: The Story of Light", "#5: 1 and 1"), bty = "n", pch=15 , col=colors_in , text.col = "grey", cex=0.75, pt.cex=1.5)
```
</details>

```{r echo=FALSE, fig.align="center"}
fmsb::radarchart(radarsummary[,-1] , axistype=0 , maxmin=FALSE, pcol=colors_border , pfcol=colors_in , plwd=4 , plty=1, cglcol="grey", cglty=1, axislabcol="black", cglwd=0.8, vlcex=0.8)
legend("bottomleft", legend = c("#1: Live Concerts", "#2: Peak Time", "#3: SHINee Brand", "#4: The Story of Light", "#5: 1 and 1"), bty = "n", pch=15 , col=colors_in , text.col = "grey", cex=0.75, pt.cex=1.5)
```
...
cbPalette <- c(adjustcolor("forestgreen",alpha.f=0.5), adjustcolor("darkgoldenrod2",alpha.f=0.5), adjustcolor("firebrick4",alpha.f=0.5))

# ggplot(rs, aes(x = feature, y = mean, scale_fill_manual(values=cbPalette))) + geom_polygon(aes(group = language, color = language), fill="#88d8c0", size = 1, alpha=0.2) + geom_line(aes(group = language, color = language), size = 1) + xlab("SHINee") +  ylab("shinee") + ylim(-5,1) + guides(color = guide_legend(ncol=2)) +  plotradar() + guides(colour=guide_legend(nrow=4, byrow=TRUE), shape=guide_legend(nrow=1, byrow=TRUE)) + scale_colour_manual(values=cbPalette) + theme(axis.ticks.y = element_blank(), axis.text.y = element_blank(), legend.key = element_blank(), legend.title = element_blank(), legend.background = element_rect(color="#ffffff", fill="transparent"), panel.background = element_rect(fill = "white", colour = "white", size = 0.1, linetype = "solid"), panel.grid.major = element_line(size = 0.1, linetype = 'solid', colour = "#dddddd"))
```
</details>
...
In conclusion, this Tidyverse recipe demonstated how to utilize `ggplot2` to create informative vizualizations that can convey concepts quickly and in an easily understandable way. Data visualization is considered a tool that can bridge the communication gap between a domain and its audience. This recipe demonstrated how it can helps identify areas that need attention or improvement, how it enables clarification of factors influencing attributes, and this will further allow better understandings of how things have been and will become based on the visualizations of trends.
